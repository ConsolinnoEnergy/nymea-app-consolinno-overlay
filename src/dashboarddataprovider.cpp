...
bool DashboardDataProvider::fetchingKpiSeries() const
{
    return m_fetchingKpiSeries;
}

void DashboardDataProvider::fetchKpiSeries(const QVariantList &periods)
{
    if (!m_engine || !m_engine->jsonRpcClient() || !m_engine->jsonRpcClient()->connected()) {
        qCWarning(dcDashboardDataProvider()) << "Cannot fetch KPI series: no engine or not connected.";
        return;
    }
    ...
    for (int i = 0; i < periods.count(); ++i) {
        ...
        qCDebug(dcDashboardDataProvider()) << "KPI series bar" << i
...
void DashboardDataProvider::kpiSeriesBarResponse(int commandId, const QVariantMap &data)
{
    ...
    if (!data.contains("error") && !data.contains("energyError")
        ...
        qCWarning(dcDashboardDataProvider()) << "KPI series bar" << barIndex
                                             << "request failed or missing fields:"
                                             << data;
    }

    qCDebug(dcDashboardDataProvider()) << "KPI series bar" << barIndex
