    ...
    FolderListModel {
        ...
        onCountChanged: {
            for (var i = 0; i < count; i++) {
                var item = folderModel.get(i, "fileName");

                if(item.includes("consolinno-energy.")){
                    var itemArr = item.split(".");
                    if(!languageArr.includes(itemArr[1])){
                        languageArr.push(itemArr[1])
                    }
                }
            }
            console.error(languageArr.length)
        }
    }
